...
public class EnemyWithBomb extends Entity {
  ...
  private int BombCountOfEnemy = 1;
...
//        PlaceBomb();
    if (!this.ExposeToBom && BombCountOfEnemy >= 1 && BombCountOfEnemy < 2) {
    ...
    if (BombCountOfEnemy == 0 && BombermanGame.getBombListOfEnemy().size() == 0) {
      BombCountOfEnemy = 1;
    }
    ...
    if (ExposeToBom) {
      if (_timeToDisapear != 0) {
        ...
        animate();
        _timeToDisapear--;
        this.img = Sprite.MovingSprite(Sprite.minvo_dead, Sprite.mob_dead1, Sprite.mob_dead2,
...
//System.out.println(" index " + IndexXOfBomb + " " + IndexYOfBomb);
      if ((this.getX() / Sprite.SCALED_SIZE) != IndexXOfBomb
          || (this.getY() / Sprite.SCALED_SIZE) != IndexYOfBomb) {
