        ...
        if (!_alive) {
            ...
            if (ExposeToBom) {
                if (_timeToDisapear != 0) {
                    ...
                    animate();
                    _timeToDisapear--;
                    this.img = Sprite.MovingSprite(Sprite.doll_dead, Sprite.mob_dead1, Sprite.mob_dead2, Sprite.mob_dead3,_animate, 60).getFxImage();
                }
            ...
            if (BombermanGame.getBombList().get(i).is_exploded()) {
                ...
                for (int j = 0; j < fl.length; j++) {
                    ...
                    for (int k = 0; k < fls.length; k++) {
                        ...
                        if (fls[k].getX() / Sprite.SCALED_SIZE == (this.getX()+14*2) / Sprite.SCALED_SIZE && fls[k].getY() / Sprite.SCALED_SIZE ==
                                (this.getY()) / Sprite.SCALED_SIZE) {
                            this.kill();
                        }
                    }
