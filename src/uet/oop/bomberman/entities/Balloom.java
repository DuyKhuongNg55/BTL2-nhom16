...
//  }
  ...
  public void update() {
    ...
    if (ExposeToBom) {
        if (_timeToDisapear != 0) {
            ...
            animate();
            _timeToDisapear--;
            this.img = Sprite.MovingSprite(Sprite.balloom_dead, Sprite.mob_dead1, Sprite.mob_dead2,
                Sprite.mob_dead3, _animate, 60).getFxImage();
        }
        //else BombermanGame.getEntities().remove(this);
        else {
            BombermanGame.getEntitiesRemove().add(this);
        }
        ...
        for (int j = 0; j < fl.length; j++) {
          ...
          for (int k = 0; k < fls.length; k++) {
            ...
            if (fls[k].get_direction() == 1) {
              if (Math.abs(this.getX() - fls[k].getX()) <= 15 * 2 && Math.abs(this.getX() - fls[k].getX()) > 0
                  && fls[k].getY() / Sprite.SCALED_SIZE ==
                  (this.getY()) / Sprite.SCALED_SIZE) {
                this.kill();
      ...
      if (BombermanGame.getBombList().get(i).is_exploded()) {
        ...
        for (int j = 0; j < fl.length; j++) {
          ...
          for (int k = 0; k < fls.length; k++) {
            //fls[k].set_animate(BombermanGame.getBombList().get(i).get_animate());
            if (fls[k].get_direction() == 3) {
              if (Math.abs(fls[k].getX() - this.getX()) <= 15 * 2 && Math.abs(fls[k].getX() - this.getX()) > 0
                  && fls[k].getY() / Sprite.SCALED_SIZE ==
                  (this.getY()) / Sprite.SCALED_SIZE) {
                this.kill();
                this.ExposeToBom = true;
      ...
      if (BombermanGame.getBombList().get(i).is_exploded()) {
        ...
        for (int j = 0; j < fl.length; j++) {
          ...
          for (int k = 0; k < fls.length; k++) {
            ...
            if (fls[k].get_direction() == 0) {
              if (fls[k].getX() / Sprite.SCALED_SIZE == (this.getX()) / Sprite.SCALED_SIZE
                  && Math.abs(this.getY() - fls[k].getY()) <= 16 * 2
                  && Math.abs(this.getY() - fls[k].getY()) >= 0) {
                this.kill();
                this.ExposeToBom = true;
              }
            }
      ...
      if (BombermanGame.getBombList().get(i).is_exploded()) {
        ...
        for (int j = 0; j < fl.length; j++) {
          ...
          for (int k = 0; k < fls.length; k++) {
            ...
            if (fls[k].get_direction() == 2) {
              if (fls[k].getX() / Sprite.SCALED_SIZE == (this.getX()) / Sprite.SCALED_SIZE
                  && Math.abs(this.getY() - fls[k].getY())
                  <= 14 * 2 && Math.abs(this.getY() - fls[k].getY())
                  > 0) {
                this.kill();
                this.ExposeToBom = true;
              }
  ...
  public void createMatrix() {
    for (int i = 0; i < BombermanGame.getStillObjects().size(); i++) {
      ...
      if (BombermanGame.getStillObjects().get(i) instanceof Wall || BombermanGame.getStillObjects()
          .get(i) instanceof Brick) {
    ...
    for (int i = 0; i < BombermanGame.getEntities().size(); i++) {
      if (BombermanGame.getEntities().get(i) instanceof Oneal || BombermanGame.getEntities()
          .get(i) instanceof EnemyWithTwoLife ||
